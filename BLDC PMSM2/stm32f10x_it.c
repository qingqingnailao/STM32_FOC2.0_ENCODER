...
void ADC1_2_IRQHandler(void)
{
  ...
  if((ADC1->SR & ADC_FLAG_JEOC) == ADC_FLAG_JEOC)//ADC÷–∂œ
  {
    //It clear JEOC flag
    ...
    if (SVPWMEOCEvent())
    {
      ...
      switch (State)
      {
        ...
        MCDAC_Update_Output();
		//DAC_SetDualChannelData(DAC_Align_12b_R,(Stat_Curr_q_d.qI_Component1+32768)>>4,(hTorque_Reference+32768)>>4);
      #endif    
    }
  }
  else 
  {
    ...
    #endif  
    {
     ...
     wGlobal_Flags |= BRAKE_ON;
#else
     if(MCL_Chk_BusVolt()==OVER_VOLT)  // ∑¿÷π∏…»≈
     {;}
